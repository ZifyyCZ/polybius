...
public class PolybiusOptionsScreen extends Screen {
    ...
    protected void init() {
        super.init();
        this.inputColor = new TextFieldWidget(this.textRenderer, 10, 70, 120, 14, Text.of(""));
        this.inputColor.setMaxLength(6);
        this.inputColor.setText(PolybiusClient.colorToHex(PolybiusClient.OPTIONS.buddyColor()));
        this.checkBoxBold = new CheckboxWidget(10, 100, 300, 20, Text.of("§7Make names of buddies bold"), PolybiusClient.OPTIONS.buddyBold, true);
        this.buttonPlayers = new PressableTextWidget(10, 40, 300, 14, Text.of("§aClick here to toggle between buddy list and config."), this::toggleBuddyList, this.textRenderer);
        this.addDrawableChild(new PressableTextWidget(10, 30, 300, 14, Text.of("§cClick here to refresh the list (might lag the game)"), this::refresh, this.textRenderer));
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int x = 10;
        if(showPlayers){
            drawTextWithShadow(matrices, this.textRenderer, Text.of("§fAll of the buddies ("+PolybiusClient.BUDDIES.all().size()+")"), x, 150, 0xFFFFFF);
            for (Buddy buddy : PolybiusClient.BUDDIES.all()) {
                ...
                drawTextWithShadow(matrices, this.textRenderer, Text.of("§7"+name), x, y + 160, 0xFFFFFF);
                y += 10;
            }
        }
        drawTextWithShadow(matrices, this.textRenderer, Text.of("§7Nametag color of buddies (HEX without the #) - default: #19E619"), 10, 60, -1);
        if(this.colorError == 1){
            drawTextWithShadow(matrices, this.textRenderer, Text.of("Invalid color!"), 135, 73, 0xEC3223);
        }
        drawTextWithShadow(matrices, this.textRenderer, Text.of("DISCLAIMER: i'm too lazy to make this screen look actually good"), 10, 10, 1191182335);
    }
