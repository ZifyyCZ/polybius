...
import net.zifyy.mods.polybius.PolybiusOptions;
...
//        this.checkBoxBold = new CheckboxWidget(10, 100, 300, 20, Text.literal("§7Make names of buddies bold"), PolybiusClient.OPTIONS.buddyBold, true);
        this.checkBoxBold = CheckboxWidget.builder(Text.literal("§7Make names of buddies bold"), this.textRenderer).pos(10, 100).checked(PolybiusClient.OPTIONS.buddyBold).build();
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int x = 10;
        if(showPlayers){
            context.drawText(this.textRenderer, "§fAll of the buddies ("+PolybiusClient.BUDDIES.all().size()+")", x, 150, 0xFFFFFF, true);
            for (Buddy buddy : PolybiusClient.BUDDIES.all()) {
                ...
                if(y + 160 + 10 >= context.getScaledWindowHeight()){
                    y = 0;
                    x += 150;
                }

                context.drawText(this.textRenderer, "§7"+name, x, y + 160, 0xFFFFFF, true);
                y += 10;
            }
        }
        context.drawText(this.textRenderer, "§7Nametag color of buddies (HEX without the #) - default: #19E619", 10, 60, -1, false);
        if(this.colorError == 1){
            context.drawText(this.textRenderer, "Invalid color!", 135, 73, 0xEC3223, false);
        }
        context.drawText(this.textRenderer, "DISCLAIMER: i'm too lazy to make this screen look actually good", 10, 10, ColorHelper.Abgr.withAlpha(70, 0xFFFFFF), true);
    }
